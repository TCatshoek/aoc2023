...
fn find_galaxies(world: &World) -> Vec<UVec2> {
    ...
    for y in 0..world.height {
        ...
        for x in 0..world.width {
            match world.get(x, y) {
                None => {}
                Some('.') => {}
                Some('#') => {galaxies.push(UVec2::new((x + expansion_x) as u32, (y + expansion_y) as u32))}
                Some(c) => unimplemented!("{}", c)
            };

            if empty_cols.contains(&x) { expansion_x += 1}
        }
        if empty_rows.contains(&y) { expansion_y += 1}
    }
...
fn main() {
    ...
    let galaxies = find_galaxies(&world);

    let result: u32 = galaxies.iter()
...
#...#.....";
    ...
    fn test_manhattan_distance() {
        assert_eq!(manhattan_distance(UVec2::new(1, 6), UVec2::new(5, 11)), 9);
        assert_eq!(manhattan_distance(UVec2::new(5, 11), UVec2::new(1, 6)), 9);
    }
